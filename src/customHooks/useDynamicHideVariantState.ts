...
const useDynamicHideVariantState = () => {
  const [states, setStates] = useState<any>([true]);

  const addNewState = useCallback(() => {
    setStates((prevStates: any)=> [...prevStates, true]);
  }, []);
...
export default useDynamicHideVariantState;